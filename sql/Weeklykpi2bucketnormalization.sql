-- create table `dap_ds_poweruser_playground.collectionkpisecondbucketnormalization` as 
insert into `dap_ds_poweruser_playground.collectionkpisecondbucketnormalization`
WITH
  dt AS (
  SELECT
    DATE('2023-01-30') WkstartDate,
    DATE('2023-02-05') WkendDate),
  MOBI AS (
  SELECT
  ...
  ON
    ...
    WHERE
      ...
      SELECT
        ...
        CASE
          WHEN COALESCE(weekstart.numbucket, 0) = 2 AND COALESCE(weekend.numbucket, 0) = 0 THEN weekstart.loanAccountNumber
      END
        Normalized
      FROM
        weekstart,
