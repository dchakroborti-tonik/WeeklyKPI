-- create table `dap_ds_poweruser_playground.collection_Monthly_kpi_v3` as 
insert into `dap_ds_poweruser_playground.collection_Monthly_kpi_v3` 
WITH
  dt AS (
  SELECT
    DATE('2023-03-01') WkstartDate,
    DATE('2023-03-19') WkendDate),
...
from `risk_credit_mis.loan_master_table`, dt where date_trunc(disbursementDateTime, day) <= dt.WkendDate
-- between dt.WkstartDate and dt.WkendDate 
and coalesce(loanPaidStatus, 'NA') not in ('Settled','Completed')
)
,
wc1 as 
(
