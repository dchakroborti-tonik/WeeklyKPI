  ...
  dt AS (
    SELECT
    DATE('2023-01-01') WkstartDate,
    DATE(current_date()) WkendDate),
...
where date(bucketDate) = date(dt.WkstartDate)
...
left join `risk_credit_mis.loan_master_table` lmt on lmt.loanAccountNumber = lbfrc.loanAccountNumber
left join cd on cd.customer_id = lmt.customerId
where date(bucketDate) = date_sub(date(dt.WkendDate), interval 1 day)
and date(lbfrc.firstDueDate) <= date(dt.WkendDate)
and cd.rnk = 1
)
, 
ptpstatus as 
...
buck1normalized as 
(select buck1.WkstartDate, buck1.WkendDate, buck1.loanAccountNumber , buck1end.loanAccountNumber loanid, buck1.DPD weekstartDPD, buck1end.DPD weekendDPD
...
, case when coalesce(buck1.DPD, 0) between 1 and 30 and coalesce(buck1end.DPD, 0) <= 30 then buck1.newmobilenumber end Normalized_greg 
...
, case when coalesce(buck2.DPD, 0) between 31 and 60 and coalesce(buck1end.DPD, 0) <= 60 then buck2.newmobilenumber end Normalized_greg 
...
,
buck1ptpamt as 
(select buck1normalized.WkstartDate,buck1normalized.WkendDate,loanAccountNumber
, case when Normalized is not null then (select sum(ptppaymentamount) from Ptp_payment where Loan_account_no = buck1normalized.loanAccountNumber and date(contact_date) 
between dt.WkstartDate and dt.WkendDate) else 0 end ptpamount_normalized  
,(select sum(ptppaymentamount) from Ptp_payment where Loan_account_no = buck1normalized.loanAccountNumber and date(contact_date) 
between dt.WkstartDate and dt.WkendDate)  ptpamount  
from buck1normalized, dt)
,
buck2ptpamt as 
(select buck2normalized.WkstartDate,buck2normalized.WkendDate,loanAccountNumber
...
(select (count(distinct apc_m)/count(distinct mobileNumber)) wccontactrate from wc) welcomecallcontactrate,
(select count(distinct apc_m)/count(distinct mobilefrombuckettable) Bucket1callcontactrate from buck1cr1base where mobilefrombuckettable is not null) Bucket1callcontactrate,
(select count(distinct Normalized) from buck1normalized) bucket1normalized,
...
(select count(distinct Normalized_greg) from buck1normalized)/(select count(distinct Normalized_greg_denominator) from buck1normalized) bucket1normalized_greg_percentage,
(select count(distinct Normalizedwithoutptp) from buck1normalized) bucket1normalizedwithoutptp,
(select count(distinct Normalized) from buck2normalized) bucket2normalized,
(select count(distinct Normalized_greg_denominator) from buck2normalized)Normalized_greg_denominator,
(select count(distinct Normalized_greg) from buck2normalized) bucket2normalized_greg,
(select count(distinct Normalized_greg) from buck2normalized)/(select count(distinct Normalized_greg_denominator) from buck2normalized) bucket2normalized_greg_percentage ,
(select count(distinct Normalizedwithoutptp) from buck2normalized) bucket2normalizedwithoutptp,
(select count(distinct Kept_ptp) from buck1normalized) + (select count(distinct Kept_ptp) from buck2normalized) kept_ptp,
(select sum(ptpamount) from buck1ptpamt) + (select sum(ptpamount) from buck2ptpamt) totalmoneycollectedptp,
...
with yb as 
(select 
min(WkstartDate) WkstartDate
, max(WkendDate)WkendDate
, (sum(apc_m_wc)/sum(wccontactrate))welcomecallcontactrate  
, (sum(apc_m_bucket1)/sum(Bucket1callcontact))Bucket1callcontactrate
, (sum(bucket1normalized_greg)/sum(bucket1normalized_denominator))bucket1normalized_greg_percentage
, (sum(bucket2normalized_greg)/sum(Normalized_greg_denominator))bucket2normalized_greg_percentage
, (sum(kept_ptp)) kept_ptp
, (sum(totalmoneycollectedptp))totalmoneycollectedptp
from `dap_ds_poweruser_playground.collection_Monthly_kpi_v2` 
),
fte as 
(select distinct agentFullName FROM
  `risk_credit_mis.call_attempt_history_gensys` g,
...
select *  from `dap_ds_poweruser_playground.collection_Monthly_kpi_v3` ;
...
from `dap_ds_poweruser_playground.collection_Monthly_kpi_v3` group by 1,2
),
fte as 
(select distinct agentFullName FROM
  `risk_credit_mis.call_attempt_history_gensys` g,
