 -- create table `dap_ds_poweruser_playground.collection_weekly_kpi_v4` as 
insert into `dap_ds_poweruser_playground.collection_weekly_kpi_v4`
WITH
  dt AS (
  SELECT
    DATE_SUB(CURRENT_DATE(), INTERVAL EXTRACT(DAYOFWEEK FROM CURRENT_DATE()) + 6 DAY) AS WkstartDate,
    DATE_SUB(CURRENT_DATE(), INTERVAL EXTRACT(DAYOFWEEK FROM CURRENT_DATE()) DAY) AS WkendDate),
