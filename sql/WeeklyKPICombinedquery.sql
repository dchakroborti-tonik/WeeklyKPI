-- drop table if exists dap_ds_poweruser_playground.collection_weekly_kpi;

-- create table `dap_ds_poweruser_playground.collection_weekly_kpi` as 
-- insert into `dap_ds_poweruser_playground.collection_weekly_kpi`
WITH
  dt AS (
  SELECT
    DATE('2023-03-06') WkstartDate,
    DATE('2023-03-12') WkendDate),
...
Left join buck1 on buck1.newmobilenumber = right(g.mobileNumber,10)
LEFT JOIN
...
, case when coalesce(buck2.DPD, 0) between 31 and 60 and coalesce(buck1end.DPD, 0) <= 60 and buck2.loanAccountNumber in (select distinct Loan_account_no from ptpstatus) then buck2.newmobilenumber end Normalized 
, case when coalesce(buck2.DPD, 0) between 31 and 60 and coalesce(buck1end.DPD, 0) <=60 and buck2.loanAccountNumber not in (select distinct Loan_account_no from ptpstatus) then buck2.newmobilenumber end Normalizedwithoutptp 
from buck2
...
,
buck1ptpamt as 
(select buck1normalized.WkstartDate,buck1normalized.WkendDate,loanAccountNumber, case when Normalized is not null then (select sum(ptppaymentamount) from Ptp_payment where Loan_account_no = buck1normalized.loanAccountNumber and date(contact_date) 
between dt.WkstartDate and dt.WkendDate) else 0 end ptpamount  from buck1normalized, dt)
,
buck2ptpamt as 
(select buck2normalized.WkstartDate,buck2normalized.WkendDate,loanAccountNumber, case when Normalized is not null then (select sum(ptppaymentamount) from Ptp_payment where Loan_account_no = buck2normalized.loanAccountNumber and date(contact_date) 
between dt.WkstartDate and dt.WkendDate) else 0 end ptpamount  from buck2normalized, dt),
fte as 
